 ...
 */
export function useDiscoveryArea(initialArea = CURRENT_LOCATION_VALUE) {
  const explicitArea: DiscoveryAreaSlug | null =
    initialArea && isDiscoveryAreaSlug(initialArea) ? initialArea : null;
  const startArea = explicitArea ?? CURRENT_LOCATION_VALUE;
  const [area, setAreaState] = useState(startArea);
  const [position, setPosition] = useState<LatLng | null>(null);
  const [locating, setLocating] = useState(() => !explicitArea);
  ...
  useEffect(() => {
    ...
    if (explicitArea) {
      writeDiscoveryArea(explicitArea);
      queueMicrotask(() => {
        if (cancelled) return;
        setLocating(false);
        setAreaState(explicitArea);
        emitDiscoveryArea(explicitArea);
      });
      return () => {
        cancelled = true;
      };
  ...
  return { area, setArea, position, locating };
}
