...
const SEO_HUB_BUSINESS_LIMIT = 80;
 ...
 */
async function queryBusinessesForSeoSupply(supplyKey: string): Promise<{
  ...
  const { items, source } = await listPublishedBusinesses(
    SEO_HUB_BUSINESS_LIMIT,
    supplyKey,
  );
  ...
  if (mErr) log.error("seo_menu_items_failed", { message: mErr.message });
...
const loadCachedSeoSupply = unstable_cache(
  queryBusinessesForSeoSupply,
  ["seo-hub-supply"],
  {
    revalidate: PUBLIC_DATA_REVALIDATE_SECONDS,
    tags: ["published-businesses", "seo-hubs"],
  },
);

const getCachedSeoSupply = cache((supplyKey: string) => loadCachedSeoSupply(supplyKey));

export function listBusinessesForSeoHub(supplyCategorySlugs: string[]) {
  const supplyKey = [...new Set(supplyCategorySlugs)].sort().join(",");
  return getCachedSeoSupply(supplyKey);
}
